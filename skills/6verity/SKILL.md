...
只有当 P0 与 P1 均已清零、每个核心模型通过 Step 2.1 的质量完整性与成果兑现门禁、CUMCM 项目通过 Step 6.1、P2 仅为轻微问题、编译与 PDF 检查完毕时，才写 `PASS`。编译成功和数值无冲突只是必要条件，不是充分条件。若存在尚未修复的回退事项，结论必须为 `FAIL` / `blocked`；修复完成后必须重新执行 `6verity`。
