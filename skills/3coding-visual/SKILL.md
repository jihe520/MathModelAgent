...
若实现过程中实质参考、移植或改写了外部已有的完整源码、针对性程序或现成问题实现，不得无说明地将其作为本队原创实现；必须保存可核验的真实来源，记录受其影响的部分，并将需要在论文中披露或引用的来源信息交给写作与验收链。不得编造 repository、作者、URL、commit、软件来源或其他 provenance 信息；真实来源无法确认时，不得以伪造来源掩盖。本规则只针对实质参考的外部完整实现，不适用于 NumPy、SciPy、pandas、sklearn、matplotlib、networkx、Gurobi、MATLAB toolbox、常规编程语言标准库及正常依赖的软件包和 solver；不要求重写成熟软件，也不要求为所有 import 建立学术引用。
