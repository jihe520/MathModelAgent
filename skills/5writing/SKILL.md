...
- 先更新 `AI_USAGE_LOG.md` 的 5writing 阶段摘要；无 material AI event 时不重复记录普通写作交互，有实质影响时按现有 contract 追加。根据真实记录准备 `AI工具使用详情.pdf` 的内容，覆盖工具名称/版本或型号、用途和环节、主要提示方式与过程、重要输出的采纳/拒绝/修改及人工核验，可选少量典型交互；不复制完整聊天，也不得凭空重构历史。语言润色按官方“采纳、人工修改和核验主要情况（语言润色除外）”处理，但仍须如实声明使用。该 PDF 在 `6verity` 的最终 submission artifacts 冻结前统一收口一次，不在普通写作或验收交互后反复生成。
